...
function* createPeopleIterator() {
  let index = 0;
  while (true) {
    yield people[index++ % people.length]; // with this way it can loop in array infinitely
  }
}
...
nextBtn.addEventListener("click", () => {
  ...
  img.src = person.imageURL; // in DOM you can change img source with .src
  ...
  profileInfo.querySelectorAll(
    "p"
  )[1].textContent = `From ${person.age} location`;
